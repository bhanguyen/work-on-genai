...
DEFAULT_RDS_HOST = os.getenv("RDS_HOST")
DEFAULT_RDS_DATABASE = os.getenv("RDS_DATABASE")
DEFAULT_RDS_USER = os.getenv("RDS_USER")
DEFAULT_RDS_PASSWORD = os.getenv("RDS_PASSWORD")
DEFAULT_RDS_PORT = os.getenv("RDS_PORT", "5432")
        ...
        if uploaded_file is not None:
            ...
            if st.button("Submit"):
                try:
                    ...
                    client = anthropic.Client(api_key or DEFAULT_API_KEY)
                    claude = client.load_model("claude-v1.3-pytorch")

                    # Get the response from the AI model
                    response = get_response(claude, prompt)
                    st.write(response)
